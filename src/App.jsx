...
function App() {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <Popup open={!!selectedEvent} modal onClose={closeModal}>
        <div className="p-6 max-w-2xl w-full relative overflow-auto max-h-[90vh] bg-white rounded">
          <button
            onClick={closeModal}
          ...
          {selectedEvent && (isEditing ? (
            ...
            <>
              <h2 className="text-black font-bold mb-2">{selectedEvent.title}</h2>
              <ReactMarkdown remarkPlugins={[remarkGfm]}>
                {selectedEvent.description || ''}
              </ReactMarkdown>
              {selectedEvent.start && selectedEvent.end && (
                <p className="text-sm text-black-500 mb-2">Période : {selectedEvent.start.toString()} → {selectedEvent.end.toString()}</p>
              )}
              {selectedEvent.image && (
                <img src={selectedEvent.image} alt="illustration" className="my-2 max-h-48 rounded" />
              )}
              {selectedEvent.location && (
                ...
                >
                  ...
                  />
                  <Marker position={[selectedEvent.location.lat, selectedEvent.location.lng]}>
                    <Popup>{selectedEvent.location.name}</Popup>
                  </Marker>
                </MapContainer>
              )}
              {selectedEvent.linksTo?.length > 0 && (
                <div className="mt-4">
                  <h3 className="font-semibold mb-2">Événements liés :</h3>
                  <ul className="list-disc ml-5 space-y-1">
                    {selectedEvent.linksTo.map((id) => {
